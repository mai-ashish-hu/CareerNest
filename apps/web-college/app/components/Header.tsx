...
export function Header({ userName, userRole, collegeName, onMenuToggle }: { userName: string; userRole: string; collegeName?: string; onMenuToggle?: () => void }) {
  return (
        ...
        <div className="flex items-center gap-4">
          ...
          {collegeName && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-surface-800 truncate max-w-[200px] hidden sm:inline">{collegeName}</span>
            </div>
          )}
        </div>
