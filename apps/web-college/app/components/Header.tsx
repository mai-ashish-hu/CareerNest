...
export function Header({ userName, userRole, collegeName }: { userName: string; userRole: string; collegeName?: string }) {
  return (
    <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-surface-100">
      <div className="flex items-center justify-between px-8 py-4">
        ...
        <div className="flex items-center gap-6">
          {collegeName && (
            <div className="flex items-center gap-2 pr-6 border-r border-surface-200">
              ...
              <span className="text-sm font-semibold text-surface-800 truncate max-w-[200px]">{collegeName}</span>
            </div>
          )}
