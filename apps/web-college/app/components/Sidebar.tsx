...
export function Sidebar({ links, collegeName, isOpen, onClose }: { links: SidebarLink[]; collegeName?: string; isOpen?: boolean; onClose?: () => void }) {
  return (
    <>
      {/* Mobile backdrop */}
      {isOpen && (
      ...
      <aside className={cn(
        ...
        <div className="p-6 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center font-bold text-white', 'bg-gradient-to-br from-primary-600 to-indigo-500')}>CN</div>
            <div>
              <h1 className="font-bold text-lg">CareerNest</h1>
              <p className="text-xs text-surface-400 truncate max-w-[130px]">{collegeName || 'College Portal'}</p>
            </div>
          </div>
